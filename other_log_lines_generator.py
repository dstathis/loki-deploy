...
def generate_log_lines(num, words):
    return [generate_log_text(num, words) for _ in range(num)]


def generate_log_text(number, words):
    date_time = datetime.now().isoformat()
    return LOG_PATTERN.format(date_time=date_time, level=choice(LEVELS), num=number, random_text=' '.join([choice(words) for _ in range(10)]))

def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('-n', '--number', action='store', dest='num',
                        help='Number of lines per second', default=30000, type=int)
    parser.add_argument('-f', '--file', action='store', default='/tmp/loki_load_test.log', help='file to write to')
    parser.add_argument('--once', action='store_true', help='Do not loop')
...
def main():
    ...
    with open(args.file, mode='at', encoding='utf-8') as f:
        while True:
            f.write('\n'.join(generate_log_lines(args.num, words)))
            if args.once:
                break
            sleep(1)
